...
- `swipeflow/api/`, `swipeflow/models/`, `client.py`, `errors.py`, `types.py` and `__init__.py` are produced by `scripts/generate.py` from the OpenAPI spec. Never edit them by hand (`__init__.py` comes from `templates/package_init.py.jinja`).
- `swipeflow/_wrapper.py` is the only hand-written SDK code (authentication, defaults, error raising). Endpoint- or model-specific code does not belong there.
...
Commit the resulting changes to the generated files in `swipeflow/`. Method names come from each operation's `operationId` in the spec.
...
4. **Code Quality**: Ensure
   - the generated files in `swipeflow/` match `python scripts/generate.py` output
